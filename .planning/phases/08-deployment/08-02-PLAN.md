...
7. Store the Railway domain for verification (will be used in subsequent steps).

Note: First deployment may take 5-10 minutes due to ML model dependencies.
  </action>
  <verify>
  After deployment completes, run `railway domain` to get the actual URL, then:
  ```bash
  # Store railway domain in shell variable
  RAILWAY_URL=$(railway domain 2>/dev/null | grep -o 'https://[^ ]*' | head -1)
  echo "Railway URL: $RAILWAY_URL"

  # Test health endpoint
  curl -s "${RAILWAY_URL}/health" | grep -q "healthy" && echo "Backend healthy"

  # Test CORS preflight
  curl -s -I -X OPTIONS "${RAILWAY_URL}/estimate" \
  ...
  <done>
    ...
    - Backend URL available and stored for Task 2
  </done>
</task>
...
3. Set environment variables via Vercel dashboard or CLI:
   ...
   # Enter: the Railway URL from Task 1
...
6. Update Railway CORS to include Vercel domain:
   ```bash
   cd ../backend
   railway variables set CORS_ORIGINS='["https://<vercel-domain>","http://localhost:3000"]'
   railway up  # Redeploy with new CORS
   ```
  </action>
  <verify>
  After deployment completes, run `vercel ls` to get the actual URL, then:
  ```bash
  # Get vercel domain
  VERCEL_URL=$(vercel ls 2>/dev/null | grep -o 'https://[^ ]*\.vercel\.app' | head -1)
  echo "Vercel URL: $VERCEL_URL"

  # Check frontend loads (returns HTML)
  curl -s "${VERCEL_URL}" | head -20

  # Check login page accessible
  curl -s -L "${VERCEL_URL}/login" | grep -i "password" && echo "Login page works"
  ```
  </verify>
  <done>
