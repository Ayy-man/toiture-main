...
3. Fix `backend/app/main.py` imports:
   - Change `from backend.app.*` to `from app.*` (Railway runs from backend/ directory)
   - Update all import statements: config, routers, services
...
5. Update `backend/.env.example` with all required variables:
  ...
  # Verify imports are fixed
  grep -c "from app\." backend/app/main.py
  ...
  <done>
    ...
    - main.py imports use `from app.*` pattern
...
- [ ] backend/app/main.py imports work standalone (not from backend.*)
...
3. Imports are compatible with Railway deployment structure
